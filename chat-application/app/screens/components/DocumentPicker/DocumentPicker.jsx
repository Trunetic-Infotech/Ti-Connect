...
// components/DocumentPicker/DocumentPickerModal.jsx
import React, { useEffect, useState } from "react";
import {
  View,
  Text,
...
const DocumentPickerModal = ({ visible, onSend, onClose }) => {
  ...
  const openDocumentPicker = async () => {
    try {
      ...
      if (result.type === "cancel") return onClose?.();

      let docsArray = result.assets || (Array.isArray(result) ? result : [result]);
      if (docsArray.length > 5) {
        Alert.alert("Limit Exceeded", "You can select up to 5 documents only.");
        docsArray = docsArray.slice(0, 5);
      }
