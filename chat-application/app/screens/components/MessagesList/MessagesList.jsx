          ...
          <TouchableWithoutFeedback onLongPress={() => handleLongPress(item)}>
            ...
            >
              ...
              {item.message_type === "image" ? (
                <TouchableOpacity
                  onPress={() => handleOpenMedia("image", item.media_url)}
                >
                  <Image
                ...
                <TouchableOpacity
                  onPress={() =>
                    handleOpenMedia("audio", item.media_url, {
                ...
                <TouchableOpacity
                  onPress={() => handleOpenMedia("document", item.media_url)}
                >
                  <DocumentViewer uri={item.media_url} />
                </TouchableOpacity>
              ) : (
                <Text
                  className={`${isMe ? "text-white" : "text-gray-800"} text-[15px] leading-5`}
                >
                  {item.message || "[empty]"}{" "}
                  {item.isEdited && (
                    <Text
                      className={`text-[10px] ${isMe ? "text-white/70" : "text-gray-600"}`}
                    >
                      (edited)
                    </Text>
                  )}
              ...
              {item.created_at && (
                <View className="flex-row items-center justify-end mt-1">
                  <Text
                    className={`text-[10px] ${isMe ? "text-white/60" : "text-gray-600/70"}`}
                  >
   ...
   const handleViewableItemsChanged = useCallback(
    ({ viewableItems }) => {      if (!user || !viewableItems) return;
      ...
      viewableItems.forEach((viewable) => {
        ...
        if (!msg || !msg.id) return; // ← THIS PREVENTS THE CRASH

        // Only mark as read if it's NOT from me and not already read
        if (msg.sender_id !== user.id && msg.status !== "read") {
          ...
          setMessages((prev) =>
            prev.map((m) =>
              m && m.id === msg.id ? { ...m, status: "read", is_read: 1 } : m
            )
          );
        }
      });
